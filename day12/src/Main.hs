...
data Three a = Three a a a deriving (Eq, Ord, Show, Foldable, Traversable, Functor, Generic1)
  deriving Applicative via Generically1 Three

newtype Present = Present (Three (Three Bool)) deriving Show
...
slack ps (Problem (h, w) cardinalities) = area - footprint
  where area = h * w
        ...
        size (Present p) = length . filter id . (>>= toList) . toList $ p
...
prepare = fromMaybe (error "no parse") . (=~ input)
  where input = Input <$> many (present <* sym '\n') <*> many (problem <* sym '\n')
        present = label *> (Present <$> sequenceA (pure row))
